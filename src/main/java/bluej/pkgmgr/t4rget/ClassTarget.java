...
import bluej.pkgmgr.t4rget.role.*;
...
public class ClassTarget extends DependentTarget
     ...
     * @throws IllegalArgumentException if the package name is not a valid java identifier
     */
    ...
    private void setTypeParameters(ClassInfo info) {
        StringBuilder newTypeParameters = new StringBuilder();
        if (info.hasTypeParameter()) {
            Iterator<String> i = info.getTypeParameterTexts().iterator();
            newTypeParameters = new StringBuilder("<" + i.next());

            while (i.hasNext()) {
                newTypeParameters.append(",").append(i.next());
            }
            newTypeParameters.append(">");
        }
        if (!newTypeParameters.toString().equals(typeParameters)) {
            typeParameters = newTypeParameters.toString();
            updateDisplayName();
        }
    }
    ...
    public void analyseDependencies(Class<?> cl) {
        if (cl != null) {
            ...
            for (Class<?> anInterface : interfaces) {
                addInterface(anInterface.getName());
            }
        }
    }
    ...
    private void putFXLaunchResult(PackageEditor ed, Window fxWindow, CompletableFuture<FXPlatformSupplier<DebuggerResult>> result) {
        result.thenAccept(supplier -> Platform.runLater(() -> {
            DebuggerResult r = supplier.get();
            if (r.getExitStatus() == Debugger.NORMAL_EXIT) {
    ...
    private class CompileAction extends MenuItem {
        public CompileAction(boolean enable) {
            super(compileStr);
            setOnAction(e -> getPackage().compile(ClassTarget.this, CompileReason.USER, CompileType.EXPLICIT_USER_COMPILE));
            setDisable(!enable);
            JavaFXUtil.addStyleClass(this, MENU_STYLE_INBUILT);
        }
    }
    ...
    public class ConvertToJavaAction extends MenuItem {
        ...
        private void actionPerformed() {
            if (JavaFXUtil.confirmDialog(
                    "convert.to.java.title",
                    "convert.to.java.message",
                    parentWindow,
                    true)) {
                convertStrideToJava();
            }
    ...
    private void duplicate() {
        ...
        DuplicateClassDialog dialog = new DuplicateClassDialog(Objects.requireNonNull(pmf).getFXWindow(), "CopyOf" + originalClassName, sourceType);
    ...
    private void addStride(TopLevelCodeElement element) {
        ...
        try {
            Files.write(strideFile.toPath(),
                    Arrays.asList(Utility.splitLines(Utility.serialiseCodeToString(element.toXML()))),
     ...
     *
     * @param isNaviviewExpanded if it's expanded
     */
