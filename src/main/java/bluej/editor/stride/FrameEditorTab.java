...
public class FrameEditorTab extends FXTab implements InteractionManager, SuggestionListParent {
    ...
    private final SimpleBooleanProperty initialised = new SimpleBooleanProperty(false);
    ...
    void showDebuggerControls(DebuggerThread thread) {
        ...
        continueButton.setOnAction(e -> {
            project.getDebugger().runOnEventHandler(() -> thread.cont());
    ...
    public Map<SuggestionList.SuggestionShown, Collection<AssistContentThreadSafe>> getImportSuggestions() {
        ...
        imports.values().forEach(p -> ret.merge(p.getKey(), new ArrayList<AssistContentThreadSafe>(Arrays.asList(p.getValue())), (a, b) -> {
            a.addAll(b);
            return a;
        }));
        return ret;
