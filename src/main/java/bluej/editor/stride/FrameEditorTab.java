...
import bluej.stride.framedjava.ast.links.*;
...
public class FrameEditorTab extends FXTab implements InteractionManager, SuggestionListParent {
    ...
    void enableCycleBirdseyeView() {
        ...
        if (viewProperty.get() == View.NORMAL && getTopLevelFrame().canDoBirdseye()) {
            ...
            birdseyeDefaultRequestFocusAfter = () -> {
                if (birdseyeDefaultFocusAfter != null) birdseyeDefaultFocusAfter.requestFocus();
            };
    ...
    @Override
    @OnThread(Tag.Any)
    public Map<SuggestionShown, Collection<AssistContentThreadSafe>> getImportSuggestions() {
        ...
        HashMap<SuggestionShown, Collection<AssistContentThreadSafe>> ret = new HashMap<>();
        imports.values().forEach(p -> ret.merge(p.getKey(), new ArrayList<AssistContentThreadSafe>(Arrays.asList(p.getValue())), (BiFunction<Collection<AssistContentThreadSafe>, Collection<AssistContentThreadSafe>, Collection<AssistContentThreadSafe>>) (a, b) -> {
            a.addAll(b);
            return a;
        }));
        return ret;
