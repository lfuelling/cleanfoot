...
public @OnThread(Tag.FX) class FXTabbedEditor
{
    ...
    private final StringProperty titleStatus = new SimpleStringProperty("");
    ...
    private void initialiseFX()
    {
        ...
        collapsibleCatalogueScrollPane = new UntitledCollapsiblePane(catalogueScrollPaneStacked, ArrowLocation.LEFT,
                PrefMgr.getFlag(PrefMgr.STRIDE_SIDEBAR_SHOWING), (b) -> {
            //TODO idk what this is for, it was only used for spying
        });
    ...
    private boolean calcDragCopy(JavaFXUtil.DragType dragType, boolean toShelf)
    {
        switch (dragType)
        {
            ...
            default:
                // Do the default:
                // Move from editor to shelf, or editor to editor
                if (dragFromShelf && toShelf)
                    return false; // Move within shelf
                else return dragFromShelf && !toShelf; // Copy from shelf to editor
        }
    }
