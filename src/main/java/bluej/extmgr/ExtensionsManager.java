...
public class ExtensionsManager
    ...
    private final List<ExtensionWrapper> extensions;
    ...
    /**
     * Loads greenfoot and extensions that are in system and user location.
     */
    private void loadExtensions() {

        loadGreenfootExtension();
    ...
    private void loadGreenfootExtension() {
        ExtensionWrapper aWrapper = new GreenfootWrapper(prefManager);

        // Loading this wrapper failed miserably, too bad...
        if (!aWrapper.isJarValid()) {
            log.error("Unable to load Greenfoot!");
            return;
        }

        // Let me see if I already have this extension loaded
        if (isWrapperAlreadyLoaded(aWrapper)) {
            log.error("Greenfoot is already loaded!");
            return;
        }

        aWrapper.newExtension(null);
     ...
     * @param project   A project this extension is bound to, or null if
     ...
     *
     * @param eventId The event id (see BlueJEvent)
