...
public class Invoker
    ...
    public Invoker(Stage frame, Package pkg, CallableView member, ResultWatcher watcher,
        ...
        compiler = new InvokerCompiler() {
            public void compile(File[] files, CompileObserver observer) {
                ...
                JobQueue.getJobQueue().addJob(wrapped.toArray(new CompileInputFile[0]), project.getClassLoader(),
     ...
     * @param pmf        the frame of the package we are working on
     * @param member     the member to invoke
     * @param debuggerObject the object to invoke the method on
     * @param watcher    an object interested in the result of the invocation
     */
    ...
    private File writeInvocationFile(String paramInit, String callString,
        ...
        Map<String, String> objBenchVarsMap = new HashMap<>();

        if (wrappers.hasNext() || localVars != null) {
            buffer.append("final bluej.runtime.BJMap __bluej_runtime_scope = getScope(\"")
                    .append(scopeId)
                    .append("\");")
                    .append(Config.nl);
        ...
        for (String s : objBenchVarsMap.values()) {
            buffer.append(s);
        }
        ...
        if (localVars != null) {
            for (Iterator<?> i = localVars.getValueIterator(); i.hasNext(); ) {
                ...
                if (!wrapper.isFinal() || !wrapper.isInitialized()) {
                    ...
                    buffer.append("__bluej_runtime_scope.put(\"lv:")
                            .append(instname)
                            .append("\", ");
    ...
    private void extractValue(StringBuffer buffer, String scopePx, String instname, JavaType type, String typeStr) {
        if (type.isPrimitive()) {
            ...
            buffer.append("((")
                    .append(castType)
                    .append(")__bluej_runtime_scope.get(\"");
            buffer.append(scopePx)
                    .append(instname)
                    .append("\")).")
    ...
    private void wrapValue(StringBuffer buffer, String name, JavaType type) {
        if (type.isPrimitive()) {
            if (type.typeIs(JavaType.JT_BOOLEAN))
                buffer.append("java.lang.Boolean.valueOf(").append(name).append(")");
            else if (type.typeIs(JavaType.JT_BYTE))
                buffer.append("new java.lang.Byte(").append(name).append(")");
            else if (type.typeIs(JavaType.JT_CHAR))
                buffer.append("new java.lang.Character(").append(name).append(")");
            else if (type.typeIs(JavaType.JT_DOUBLE))
                buffer.append("new java.lang.Double(").append(name).append(")");
            else if (type.typeIs(JavaType.JT_FLOAT))
                buffer.append("new java.lang.Float(").append(name).append(")");
            else if (type.typeIs(JavaType.JT_LONG))
                buffer.append("new java.lang.Long(").append(name).append(")");
            else if (type.typeIs(JavaType.JT_INT))
                buffer.append("new java.lang.Integer(").append(name).append(")");
            else if (type.typeIs(JavaType.JT_SHORT))
                buffer.append("new java.lang.Short(").append(name).append(")");
            else {
                throw new UnsupportedOperationException("unhandled primitive type.");
            }
        } else {
    ...
    private void deleteShellFiles() {
        ...
        String[] innerClassFiles = pkgPath.list((dir, name) -> (name.startsWith(shellName + "$")));
