...
import bluej.pkgmgr.t4rget.ClassTarget.SourceFileInfo;
...
public class DataCollectorImpl
{
            ...
            for (SourceFileInfo fileInfo : ct.getAllSourceFilesJavaLast())
            {
                ...
                switch (fileInfo.sourceType)
                {
                    case Java:
                        ...
                        break;
                    case Stride:
                        mpe.addPart("project[source_files][][source_type]", CollectUtility.toBody("lang/stride"));
                        break;
                }
    ...
    static void edit(final Package pkg, List<EditedFileInfo> editedFiles)
    {
        ...
        for (EditedFileInfo editedFile : editedFiles)
        {
            ...
            editedFile.anonSource = Arrays.asList(Objects.requireNonNull(
                    Utility.splitLines(CodeAnonymiser.anonymise(editedFile.source))));
        }
                
        submitEvent(proj, pkg, EventName.EDIT, new Event() {
            
            //Edit solely within one line
            private boolean isOneLineDiff(Patch<Delta> patch)
            {
    ...
    protected static String makeDiff(Patch<Delta> patch)
    {
        ...
        for (Delta delta: patch.getDeltas()) {
            ...
            diff.append("@@ -")
                    .append(srcLine).append(",")
                    .append(srcSize).append(" +")
                    .append(destLine).append(",")
                    .append(destSize).append(" @@\n");
    ...
    {
        MultipartEntity mpe = new MultipartEntity();
        mpe.addPart("event[thread_name]", Objects.requireNonNull(CollectUtility.toBody(threadName)));
    ...
    {
        MultipartEntity mpe = new MultipartEntity();
        mpe.addPart("event[codepad][outcome]", Objects.requireNonNull(CollectUtility.toBody("success")));
        mpe.addPart("event[codepad][command]", Objects.requireNonNull(CollectUtility.toBody(command)));
        mpe.addPart("event[codepad][result]", Objects.requireNonNull(CollectUtility.toBody(output)));
    ...
    {
        MultipartEntity mpe = new MultipartEntity();
        mpe.addPart("event[codepad][outcome]", Objects.requireNonNull(CollectUtility.toBody("error")));
        mpe.addPart("event[codepad][command]", Objects.requireNonNull(CollectUtility.toBody(command)));
        mpe.addPart("event[codepad][error]", Objects.requireNonNull(CollectUtility.toBody(error)));
        submitEvent(pkg.getProject(), pkg, EventName.CODEPAD, new PlainEvent(mpe));
    }
    ...
    {
        MultipartEntity mpe = new MultipartEntity();
        mpe.addPart("event[codepad][outcome]", Objects.requireNonNull(CollectUtility.toBody("exception")));
        mpe.addPart("event[codepad][command]", Objects.requireNonNull(CollectUtility.toBody(command)));
        mpe.addPart("event[codepad][exception]", Objects.requireNonNull(CollectUtility.toBody(exception)));
        submitEvent(pkg.getProject(), pkg, EventName.CODEPAD, new PlainEvent(mpe));
    }
